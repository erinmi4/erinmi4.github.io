...
这份笔记的核心在于**打破死记硬背的局限，提炼出解题的底层通用逻辑**。推荐你把它当作方法论指南，在做题前反复对照。
...
> **一句话核心判定：**
>
> 当方程中出现某个**复合量 $u = \varphi(x,y)$**，并且通过其导数关系 $u' = \varphi_x + \varphi_y y'$ 能够把原方程中的 $y$ 和 $y'$ **完全消掉或转换**时，就可以通过令 $u = \varphi(x,y)$ 将方程转化为关于 $u$ 和 $x$ 的新方程 $u' = F(x,u)$。如果能进一步整理为 $g(u)du = f(x)dx$，则成功化为**可分离变量方程**。

我们常说的“齐次方程”、“$ax+by+c$ 型”都只是这个底层逻辑下的特例。

##### 🔍 核心技能 1：代换的“识别入口”（候选表）

看到以下标志性复合量时，应条件反射般地想到对应的代换方向。**注意：不要死记公式，要理解它们的导数联动关系！**

| **复合量特征**                     | **优先代换设法**                  | **导数/变量关系（消去 y,y′ 的武器）**        | **对应经典题型**       |
| ---------------------------------- | --------------------------------- | -------------------------------------------- | ---------------------- |
| **$\frac{y}{x}$ 或 $\frac{x}{y}$** | $u = \frac{y}{x} \implies y = ux$ | $y' = u'x + u$                               | 齐次方程 (Homogeneous) |
| **$ax + by + c$**                  | $u = ax + by + c$                 | $u' = a + by'$                               | 线性组合复合型         |
| **$xy$**                           | $u = xy \implies y = \frac{u}{x}$ | $u' = xy' + y$ 或 $y' = \frac{xu' - u}{x^2}$ | $xy$ 联动型            |
| **$x + y$ / $x - y$**              | $u = x \pm y$                     | $u' = 1 \pm y'$                              | 对称加减型             |
| **$y + px$**                       | $u = y + px$                      | $u' = y' + p$                                | 倾斜斜率型             |

##### 🛠️ 核心技能 2：做题四步标准流程（以典型例题为例）

###### 典型例题

求解微分方程：$(x^2y+x)y' + xy^2 - y = 0$
...
> **🧠 观察结论：** 括号内部只依赖于复合量 **$xy$**，且外面同时出现了 $x y'$ 与 $y$ 的结构（它们是 $(xy)'$ 的组成部分）。因此，最自然、最对称的整体代换是令 **$u = xy$**。

###### 🔄 第二步：机械代换，写出变量关系

确定 $u = xy$ 后，立刻列出用 $u, x$ 表达 $y, y'$ 的公式：
...
$$x(u+1) \cdot \left(\frac{xu' - u}{x^2}\right) + \frac{u}{x}(u-1) = 0$$

两边同时乘以 $x$ 展开并整理：

$$(u+1)(xu' - u) + u(u-1) = 0$$

$$x(u+1)u' - u^2 - u + u^2 - u = 0$$

$$x(u+1)u' - 2u = 0$$

> **✅ 检查结果：** 方程中**完全没有残留**单独的 $y$，代换完全可行！

###### 📐 第四步：分离变量并求解

将方程整理为标准的分离变量形式：

$$x(u+1)\frac{du}{dx} = 2u \implies \frac{u+1}{u}du = 2\frac{dx}{x}$$

两边同时积分：

$$\int \left(1 + \frac{1}{u}\right)du = 2\int \frac{1}{x}dx$$

$$u + \ln|u| = 2\ln|x| + C$$

最后，将 $u = xy$ **代回原变量**，即可得到最终隐式通解。

##### ⚠️ 错因剖析与避坑指南

1. **“神奇设元”的迷思：**

   绝大多数高分同学不是“一眼看出答案”才去设 $u = xy$ 的，而是通过**第一步的因式分解**看到括号内被 $xy$ 统治，进而做出的方向性尝试。

2. **盲目拆分 $dx, dy$：**

   只要原式中 $x$ 和 $y$ 还在根号、括号或分母里“纠缠”（如 $xy+1$），直接写成 $dy = f(x,y)dx$ 是没有任何操作空间的。**必须先整体换元，把纠缠态解开。**

3. **缺乏“检查”意识：**
   代换完后，如果式子里还留有无法写成 $\frac{u}{x}$ 形式的独立 $y$，说明该代换不是正确方向，应立刻返回第一步，重新观察是否能提取其他复合量。
...
> 不只有这种情况还有 $$\frac{dy}{dx} = \varphi(ax+by+c)$$ 这样的情况存在
...
微分方程形如 $\frac{dy}{dx} = f(ax+by+c)$。右端项不是独立的 $x$ 或 $y$，而是 **$x$ 与 $y$ 的线性组合复合形式**，无法直接分离变量。

##### 🔑 核心解题思路

利用**整体代换思想**，令新变量 $u = ax+by+c$，通过链式法则将原方程转化为关于 $u$ 和 $x$ 的**可分离变量微分方程**。

#### 📝 详细解析与分类讨论

**题目：** 求解微分方程 $\frac{dy}{dx} = \sqrt{ax+by+c}$ （其中 $a, b, c$ 为常数）

##### 🎈 第一阶段：变量代换（统一变形）

令 $u = ax+by+c$，两边对 $x$ 求导得：

$$\frac{du}{dx} = a + b\frac{dy}{dx}$$

将原方程 $\frac{dy}{dx} = \sqrt{u}$ 代入上式，得到关于 $u$ 与 $x$ 的新方程：

$$\frac{du}{dx} = a + b\sqrt{u}$$
...
$$\frac{du}{a + b\sqrt{u}} = dx$$

两边同时积分 $\int \frac{1}{a+b\sqrt{u}} \, du = \int dx$：

> **💡 积分小技巧（换元+拆项）：**
>
> 令 $v = \sqrt{u}$，则 $u = v^2$，$du = 2v \, dv$。
>
> $$\int \frac{2v}{a+bv} \, dv = \frac{2}{b} \int \frac{(a+bv) - a}{a+bv} \, dv = \frac{2}{b} \int \left( 1 - \frac{a}{a+bv} \right) \, dv$$
>
> 计算得：
>
> $$\frac{2}{b}v - \frac{2a}{b^2}\ln|a+bv| = x + C$$

将 $v = \sqrt{u} = \sqrt{ax+by+c}$ 代回原变量，得到**隐式通解**：

$$\boxed{\frac{2}{b}\sqrt{ax+by+c} - \frac{2a}{b^2}\ln\left|a+b\sqrt{ax+by+c}\right| = x + C}$$

##### 情形二：$b \neq 0$ 且 $a + b\sqrt{u} = 0$（特解/奇解分析）

若 $a + b\sqrt{u} = 0$，则 $\frac{du}{dx} = 0$，表明 $u$ 为常数。

- **存在条件：** $-\frac{a}{b} \ge 0$

- **求解：** $\sqrt{u} = -\frac{a}{b} \implies ax+by+c = \frac{a^2}{b^2}$

- 整理得到**特殊解**：

  $$\boxed{y = -\frac{a}{b}x + \frac{1}{b}\left(\frac{a^2}{b^2} - c\right)}$$

##### 情形三：$b = 0$（退化情形）

原方程退化为 $\frac{dy}{dx} = \sqrt{ax+c}$，右边只含 $x$，可直接积分 $y = \int \sqrt{ax+c} \, dx$。

- **当 $a \neq 0$ 时：**

  $$\boxed{y = \frac{2}{3a}(ax+c)^{\frac{3}{2}} + C}$$

- **当 $a = 0$ 时：** 方程变为 $\frac{dy}{dx} = \sqrt{c}$（需满足 $c \ge 0$）

  $$\boxed{y = \sqrt{c}x + C}$$

##### ⚠️ 常见易错点与避坑指南

- **盲目拆分 $dx$ 和 $dy$：** 看到题目切忌直接写成 $dy = \sqrt{ax+by+c}\,dx$ 然后尝试积分。只要根号内 $x$ 和 $y$ 还在“纠缠”，就必须先做整体换元。
- **遗漏特解讨论：** 在分离变量将 $a + b\sqrt{u}$ 除到分母上时，严格来说必须讨论其等于 $0$ 的情况（即上面的情形二）。虽然很多普通考试只要求写出隐式通解，但严谨的选拔性考试（如考研、竞赛）中，遗漏此处的特解会导致扣分。
