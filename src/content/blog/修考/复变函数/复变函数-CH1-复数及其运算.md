...
我们在高等数学中学习的函数（如 $y = x^2, y = \sin x$）都是定义在**实数域** $\mathbb{R}$ 上的。而**复变函数**（Complex Function），顾名思义，就是将自变量的定义域推广到了**复数域** $\mathbb{C}$ 上。

> **严格数学定义：** 设 $E$ 是复平面上的一个非空复数集合。如果有一种对应法则 $f$，使得对于集合 $E$ 中的每一个复自变量 $z = x + iy$，都有唯一确定的复因变量 $w = u + iv$ 与之对应，则称 $f$ 是定义在 $E$ 上的**复变函数**，记作：
>
> $$w = f(z) \quad (z \in E)$$

#### 2. 复变函数的双实函数表示法

由于自变量 $z$ 和因变量 $w$ 都是复数，我们可以将它们分别拆解为实部和虚部：

- 自变量：$z = x + iy \quad (x, y \in \mathbb{R})$
- 因变量：$w = u + iv \quad (u, v \in \mathbb{R})$

代入函数关系式 $w = f(z)$ 中，可以得到：

$$u + iv = f(x + iy)$$

这说明，给定了 $x$ 和 $y$，就能唯一确定 $u$ 和 $v$。因此，实部 $u$ 和虚部 $v$ 实际上是关于实变量 $x$ 和 $y$ 的二元实函数：

$$w = f(z) = u(x, y) + i v(x, y)$$

> **💡 学习感悟与起承转合：**
>
> - **直观感受：** 复变函数 $f(z)$ 表面上是一个整体，但它的背后其实是由**两个不同的二元实函数** $u(x, y)$ 和 $v(x, y)$ 共同驱动的。
> - **方法论启示：** 这一拆分给复变函数的研究提供了一条最基本的通路——**将复变函数问题转化为高等数学中我们熟悉的二元实函数问题来求解**。

### 二、 几何直观：$z$ 平面与 $w$ 平面的映射变换

在实函数中，我们可以在一个二维直角坐标系 $xOy$ 中画出 $y = f(x)$ 的函数图像。但在复变函数中，自变量 $z$ 需要 2 个维度（实部 $x$、虚部 $y$），因变量 $w$ 也需要 2 个维度（实部 $u$、虚部 $v$）。若想画出完整的函数图像，我们需要 4 个维度，这在三维空间中是无法直接画出的。

为了解决这个问题，我们引入映射（Mapping）的概念，使用两个独立的复平面来表现这种关系：

1. $z$ **平面（自变量空间）**：以 $x$ 为横轴，$y$ 为纵轴。
2. $w$ **平面（因变量空间）**：以 $u$ 为横轴，$v$ 为纵轴。

通过复变函数 $w = f(z)$，我们将 $z$ 平面上的一个点或区域，变换（映射）为 $w$ 平面上的一个点或区域。
...
**题目：** 设复变函数为 $w = z^3$。求 $z$ 平面上的三个点 $z_1 = i$，$z_2 = 1+i$，$z_3 = \sqrt{3}+i$ 在 $w$ 平面上的像（即对应的 $w$ 值）。
...
- **方法 A：代数展开法（适合较简单的复数）**
  ...
  - **对于** $z_1 = i$：
    ...
    $$w_1 = z_1^3 = i^3 = i^2 \cdot i = -i$$

    对应 $w$ 平面上的点 $(0, -1)$。

  - **对于** $z_2 = 1 + i$： 直接使用二项式展开或分步计算：

    $$(1+i)^2 = 1 + 2i + i^2 = 2i$$

    $$w_2 = (1+i)^3 = (1+i)^2(1+i) = 2i(1+i) = 2i + 2i^2 = -2 + 2i$$

    对应 $w$ 平面上的点 $(-2, 2)$。

- **方法 B：极坐标/指数法（适合含有根号、计算量大的复数）** 利用欧拉公式 $z = r e^{i\theta}$，则 $z^3 = r^3 e^{i3\theta}$。

  - **对于** $z_3 = \sqrt{3} + i$： 首先求模长 $r$ 和辐角 $\theta$：

    $$r = \sqrt{(\sqrt{3})^2 + 1^2} = 2$$

    $$\tan\theta = \frac{1}{\sqrt{3}} \implies \theta = \frac{\pi}{6}$$

    所以 $z_3 = 2 e^{i\frac{\pi}{6}}$。 对其求立方：

    $$w_3 = z_3^3 = \left(2 e^{i\frac{\pi}{6}}\right)^3 = 2^3 \cdot e^{i \left(3 \cdot \frac{\pi}{6}\right)} = 8 e^{i\frac{\pi}{2}}$$

    因为 $e^{i\frac{\pi}{2}} = \cos\frac{\pi}{2} + i\sin\frac{\pi}{2} = i$，所以：
...
直观上，当 $z$ 无限趋近于 $z_0$ 时，$f(z)$ 无限趋近于一个确定的复数 $A$，则称 $A$ 为 $f(z)$ 当 $z \to z_0$ 时的极限。

> **严格** $\varepsilon-\delta$ **定义：** 设函数 $f(z)$ 在 $z_0$ 的去心邻域 $0 < |z - z_0| < \delta_0$ 内有定义。若存在一个复常数 $A$，对于任意给定的 $\varepsilon > 0$，总存在一个 $\delta > 0 \ (0 < \delta < \delta_0)$，使得当 $0 < |z - z_0| < \delta$ 时，恒有：
>
> $$|f(z) - A| < \varepsilon$$
>
> 则称 $A$ 为 $f(z)$ 当 $z \to z_0$ 时的极限，记作 $\lim_{z \to z_0} f(z) = A$。

#### 2. 复极限与实极限的关系

复变函数极限的计算，可以通过实部和虚部分别取极限来完成： 设 $f(z) = u(x,y) + iv(x,y)$，$z_0 = x_0 + iy_0$，$A = u_0 + iv_0$。则：

$$\lim_{z \to z_0} f(z) = A \iff \lim_{(x,y)\to(x_0,y_0)} u(x,y) = u_0 \quad \text{且} \quad \lim_{(x,y)\to(x_0,y_0)} v(x,y) = v_0$$

#### 💻 例题 2：连续情况下的直接代入法

**题目：** 求极限：$\lim_{z \to 1+i} \frac{\bar{z}}{z}$

**解答与过程完善：**

- **思路分析：** 判断分母在极限点是否为 $0$。由于当 $z \to 1+i$ 时，分母 $z \to 1+i \neq 0$，该函数在 $1+i$ 处是连续的。我们可以直接将 $z = 1+i$ 代入求解。

- **计算过程：**

  $$\lim_{z \to 1+i} \frac{\bar{z}}{z} = \frac{\overline{1+i}}{1+i} = \frac{1-i}{1+i}$$

  进行分母实数化（分子分母同乘分母的共轭 $1-i$）：

  $$\frac{1-i}{1+i} = \frac{(1-i)^2}{(1+i)(1-i)} = \frac{1 - 2i + i^2}{1 - i^2} = \frac{1 - 2i - 1}{2} = \frac{-2i}{2} = -i$$

- **若拆分为二元实函数求解（对比）：** 设 $z = x+iy \implies \bar{z} = x-iy$。当 $z \to 1+i$ 时，$(x,y) \to (1,1)$：

  $$\lim_{(x,y) \to (1,1)} \frac{x-iy}{x+iy} = \frac{1-i}{1+i} = -i$$

  两种方法完全等价，但由于其连续性，直接代入显然更简洁。

#### 💻 例题 3：消去零因子法

**题目：** 求极限：$\lim_{z \to 1} \frac{z\bar{z} + 2z - \bar{z} - 2}{z^2-1}$

**解答与过程完善：**

- **思路分析：** 如果直接把 $z=1$ 代入，分母 $1^2-1=0$，分子 $1\cdot 1 + 2(1) - 1 - 2 = 0$，属于 $\frac{0}{0}$ 型未定式。这说明分子分母含有共同的零因子 $z-1$。我们需要对其进行因式分解来消去零因子。

- **计算过程：** 对分子进行分组分解：

  $$z\bar{z} + 2z - \bar{z} - 2 = (z\bar{z} - \bar{z}) + (2z - 2) = \bar{z}(z-1) + 2(z-1) = (z-1)(\bar{z}+2)$$
  ...
  $$\lim_{z \to 1} \frac{(z-1)(\bar{z}+2)}{(z-1)(z+1)}$$

  因为 $z \to 1$ 但 $z \neq 1$，故零因子 $z-1 \neq 0$，可以安全约去：

  $$\lim_{z \to 1} \frac{\bar{z}+2}{z+1}$$

  此时分母不为 $0$，直接代入 $z=1$（此时共轭 $\bar{z} = 1$）：

  $$\lim_{z \to 1} \frac{\bar{z}+2}{z+1} = \frac{1+2}{1+1} = \frac{3}{2}$$
...
不用每次都使用严格的极限定义去证明。在实际解题中，有以下两条极为实用的**黄金判定准则**：

- **准则一：利用基本初等函数的连续性（直接判定法）** 和实微积分一样，复数域下的**基本初等复函数**在其定义域内都是连续的。

  - 多项式函数：$P(z) = a_n z^n + \dots + a_0$ 在全复平面 $\mathbb{C}$ 连续。
  - 分式有理函数：$R(z) = \frac{P(z)}{Q(z)}$ 在所有使分母 $Q(z) \neq 0$ 的点处连续。
  - 指数函数 $e^z$、三角函数 $\sin z, \cos z$、共轭函数 $\bar{z}$ 均在全平面连续。
  - **结论：** 只要你的函数是由这些基本初等函数经过有限次加、减、乘、除、复合得到的，并且所求的点不在分母为零或无定义的边界上，它在这一点就必定连续！

- **准则二：拆分为实部与虚部判定（等价转化法）** 设 $f(z) = u(x,y) + iv(x,y)$。

  $$f(z) \text{ 在 } z_0 = x_0 + iy_0 \text{ 连续} \iff u(x,y) \text{ 和 } v(x,y) \text{ 两个二元实函数在 } (x_0, y_0) \text{ 处同时连续}$$

  - 这可以直接套用我们在高等数学下册中学过的二元函数连续性结论。

### 五、 重点解答：多元函数极限不存在的判定定理

> **❓ 手写笔记疑问：微积分里这部分知识我已经忘了。为什么要令** $y = kx$**？以及为什么“极限值与** $k$ **有关”就说明极限不存在？**

#### 1. 一元极限与二元（复）极限的本质区别

- **一元实函数** $\lim_{x \to x_0} f(x)$：自变量 $x$ 只能在一条一维数轴上运动。因此，趋近于 $x_0$ 的方向**只有两个**：从左边趋近（左极限）和从右边趋近（右极限）。只要左极限 = 右极限，极限就存在。
- **多元实函数（或复变函数）** $\lim_{(x,y) \to (0,0)} f(x,y)$：自变量在二维平面上运动。要趋近于原点 $(0,0)$，它的路径有**无数条**。可以是沿 $x$ 轴、沿 $y$ 轴、沿任意斜率的直线 $y = kx$、甚至可以沿着抛物线 $y = ax^2$ 绕着圈子趋近。
...
根据极限的定义，要让极限 $\lim_{z \to z_0} f(z) = A$ 存在，自变量 $z$ 必须以**任何方式、任何路径**趋近于 $z_0$ 时，其函数值 $f(z)$ 都必须趋近于**同一个唯一确定的常数** $A$。

如果自变量沿着路径 $\text{Path}_1$ 趋于 $z_0$ 时函数极限为 $A_1$，而沿着路径 $\text{Path}_2$ 趋于 $z_0$ 时函数极限为 $A_2$，且 $A_1 \neq A_2$，那么该极限就**不存在**。

#### 3. 为什么要令 $y = kx$？

为了寻找不同的趋近路径，我们最常用、最简单的方法就是采用“过原点的一族直线”。 直线族的方程为：

$$y = kx \quad (k \text{ 为直线的斜率})$$

改变 $k$ 的取值，就代表我们从不同斜率的方向（不同路径）向原点趋近：

- 当 $k = 0$ 时，表示沿 $x$ 轴（水平方向）趋近。
- 当 $k = 1$ 时，表示沿 $y = x$（对角线方向）趋近。

我们将 $y = kx$ 代入极限式中，将二元极限转化为关于 $x$ 的一元极限：

- 如果算出来的极限结果中**含有参数** $k$（即极限值随 $k$ 的改变而改变），说明： **“你从不同的方向走过来，看到的极限值是不一样的！”**
- 因此，这就违反了“路径无关性”，我们可以理直气壮地判定：**该极限不存在**。

#### 💻 例题 4：复极限不存在的判定

**题目：** 求极限：$\lim_{z \to 0} \frac{\text{Re}\{z\}}{z}$

**解答与过程完善：**

1. 写出实部和复变量代数式：

   $$\text{Re}\{z\} = x, \quad z = x + iy$$

2. 将复极限转化为二元实极限：

   $$\lim_{z \to 0} \frac{\text{Re}\{z\}}{z} = \lim_{(x,y) \to (0,0)} \frac{x}{x + iy}$$

3. **尝试沿着直线族** $y = kx$ **趋近于** $(0,0)$： 将 $y = kx$ 代入：

   $$\lim_{x \to 0} \frac{x}{x + i(kx)} = \lim_{x \to 0} \frac{x}{x(1 + ik)} = \lim_{x \to 0} \frac{1}{1 + ik} = \frac{1}{1 + ik}$$

4. **结论分析：** 计算出来的极限值 $\frac{1}{1+ik}$ **与直线的斜率** $k$ **有关**。

   - 例如：沿 $x$ 轴趋近（$k=0$），极限值为 $1$；
   - 沿对角线 $y=x$ 趋近（$k=1$），极限值为 $\frac{1}{1+i}$。 由于从不同路径趋近时的极限值不同，**故该极限不存在**。

#### 💻 例题 5：高等数学经典二元极限不存在例题

**题目：** 判定二元实极限是否存在：$\lim_{(x,y) \to (0,0)} \frac{xy}{x^2 + y^2}$

**解答过程：**

1. 令 $y = kx$，代入原式：

   $$\lim_{(x,y) \to (0,0)} \frac{xy}{x^2 + y^2} = \lim_{x \to 0} \frac{x \cdot (kx)}{x^2 + (kx)^2} = \lim_{x \to 0} \frac{k x^2}{x^2(1 + k^2)}$$

2. 消去非零因子 $x^2$（因为 $x \to 0$ 但 $x \neq 0$）：

   $$\lim_{x \to 0} \frac{k}{1+k^2} = \frac{k}{1+k^2}$$

3. 因为极限值依赖于直线斜率 $k$ 的选择（如 $k=1$ 时极限为 $\frac{1}{2}$；$k=2$ 时极限为 $\frac{2}{5}$），所以该极限**不存在**。
...
- **解析：** 设 $z = x+iy$。$\text{Im}\{z\} = y$，所以不等式等价于：

  $$y > 0$$

- **几何图形：** 表示 $z$ 平面上的**上半平面**（不包含实轴 $x$ 轴本身，用虚线表示边界）。

**(2)** $|z - 1| > 4$

- **解析：** 根据复数减法的几何意义，|z - z_1| 表示复平面上点 $z$ 到点 $z_1$ 的距离。 因此，$|z - 1| > 4$ 表示点 $z$ 到点 $1$（即坐标 $(1,0)$）的距离大于 $4$。

  *若用代数式推导：* 设 $z = x+iy$，则：

  $$|x + iy - 1| > 4 \implies |(x-1) + iy| > 4 \implies \sqrt{(x-1)^2 + y^2} > 4$$

  两边平方得到：

  $$(x-1)^2 + y^2 > 16$$

- **几何图形：** 这是一个以 $(1,0)$ 为圆心，以 $R = 4$ 为半径的圆。由于符号是“$>$”，因此它表示**该圆的外部所有区域**（不包含圆周边界本身，画图时圆周用虚线表示）。
