...
- **递归树描述**：
  ...
  - 根节点代价为 $n^3$，下一层为 $(n/2)^3$，再下一层为 $(n/4)^3$。
  - 这是一棵单支树（每个节点只有一个子节点）。
  - 总代价为等比数列：$n^3 (1 + 1/8 + 1/64 + \dots)$，首项占主导。
...
- **代入法证明**：
  ...
  $$T(n) \le c(n/2)^3 + n^3 = \frac{c}{8}n^3 + n^3 = (\frac{c}{8} + 1)n^3$$

  要使 $(\frac{c}{8} + 1)n^3 \le cn^3$，需 $\frac{c}{8} + 1 \le c$，即 $c \ge 8/7$。

- **结论**：$T(n) = O(n^3)$。

![img](./%E7%AE%97%E6%B3%95%E5%AF%BC%E8%AE%BA-CH4-%E5%88%86%E6%B2%BB%E6%B3%95.assets/v2-fc4ddd015a967582b6be80cc23a09408_r.jpg)
...
- **递归树描述**：
  ...
  - 第 $i$ 层有 $4^i$ 个节点，每个节点的代价为 $n/3^i$。
  - 第 $i$ 层的总代价为 $(4/3)^i n$。
  - 树的高度为 $\log_3 n$，叶子节点数量为 $4^{\log_3 n} = n^{\log_3 4} \approx n^{1.262}$。
...
- **代入法证明**：
  ...
  设 $T(n) \le cn^{\log_3 4} - dn$（减去低阶项以抵消 $f(n)=n$）：

  $$T(n) \le 4(c(n/3)^{\log_3 4} - d(n/3)) + n = 4 \cdot \frac{c n^{\log_3 4}}{4} - \frac{4}{3}dn + n = cn^{\log_3 4} - (\frac{4}{3}d - 1)n$$

  要使结果 $\le cn^{\log_3 4} - dn$，需 $\frac{4}{3}d - 1 \ge d$，即 $d \ge 3$。

- **结论**：$T(n) = O(n^{\log_3 4})$。

![img](./%E7%AE%97%E6%B3%95%E5%AF%BC%E8%AE%BA-CH4-%E5%88%86%E6%B2%BB%E6%B3%95.assets/v2-bb85f5c5f77a32c2cbe7e8ad136d4fab_r.jpg)
...
- **递归树描述**：
  ...
  - 第 $i$ 层总代价为 $4^i \cdot (n/2^i) = 2^i n$。
  - 叶子节点数量为 $4^{\log_2 n} = n^2$。
  - 总代价为等比数列 $n + 2n + 4n + \dots + n^2$，由末项主导。
...
- **代入法证明**：
  ...
  $$T(n) \le 4(c(n/2)^2 - d(n/2)) + n = cn^2 - 2dn + n = cn^2 - (2d - 1)n$$

  要使结果 $\le cn^2 - dn$，需 $2d - 1 \ge d$，即 $d \ge 1$。

- **结论**：$T(n) = O(n^2)$。

- ![img](./%E7%AE%97%E6%B3%95%E5%AF%BC%E8%AE%BA-CH4-%E5%88%86%E6%B2%BB%E6%B3%95.assets/v2-ffcbd27b2d651065dfb9a7ff5243fe56_r.jpg)
...
- **递归树描述**：
  ...
  - 树退化为高度为 $n$ 的高度分支树，每层代价为 $3^i$。
  - 总代价为 $\sum_{i=0}^{n-1} 3^i = \frac{3^n - 1}{2}$。

- **猜测**：$T(n) = O(3^n)$。

- **代入法证明**：

  设 $T(n) \le c3^n - d$（$d$ 为常数）：

  $$T(n) \le 3(c3^{n-1} - d) + 1 = c3^n - 3d + 1$$

  要使结果 $\le c3^n - d$，需 $3d - 1 \ge d$，即 $d \ge 1/2$。

- **结论**：$T(n) = O(3^n)$。

- ![img](./%E7%AE%97%E6%B3%95%E5%AF%BC%E8%AE%BA-CH4-%E5%88%86%E6%B2%BB%E6%B3%95.assets/v2-9c02b532e8f5926353f165690a4241dd_r.jpg)

------

**💡 归纳总结：**

- 当递归树的**每层代价逐层递减**（如 a），复杂度由**根节点**决定。
- 当递归树的**每层代价基本相等**，复杂度通常带 $\lg n$ 因子。
- 当递归树的**每层代价逐层递增**（如 b, c, d），复杂度由**叶子节点**决定。
